...
import { ReconsiderAbsenceHandler } from './reconsider-absence.handler';
import { ReconsiderAbsenceCommand } from './reconsider-absence.command';
import { Absence } from '../../domain/absence.entity';

describe('ReconsiderAbsenceHandler', () => {
  let handler: ReconsiderAbsenceHandler;
  ...
  beforeEach(async () => {
    ...
    handler = module.get<ReconsiderAbsenceHandler>(ReconsiderAbsenceHandler);
  });

  describe('RF-31: Reconsider absence from RECONSIDER state', () => {
    it('should successfully transition absence to WAITING_VALIDATION', async () => {
      ...
      const command = new ReconsiderAbsenceCommand(absenceId, userId);
    ...
    it('should throw ForbiddenException when user is not the absence creator', async () => {
      ...
      const command = new ReconsiderAbsenceCommand(absenceId, otherUserId);

      await expect(handler.execute(command)).rejects.toThrow(ForbiddenException);
      await expect(handler.execute(command)).rejects.toThrow(
        'Only the absence creator can reconsider it'
      );
    });
    ...
    it('should throw BadRequestException when absence is not in RECONSIDER state', async () => {
      ...
      await expect(handler.execute(command)).rejects.toThrow(
        `Only absences in RECONSIDER state can be resubmitted. Current status: ${AbsenceStatus.WAITING_VALIDATION}`
      );
    });
  });
});
